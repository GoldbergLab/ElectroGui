function intan_converter_to_binary_channel_files_AD(path_input, headstage_to_extract, acc_array, audio_threshold)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% intan_converter_to_binary_channel_files: Convert Intan .rhd files to
%   individual binary .nc channel files
%  
% Note by AD: this code works with 16 channel headstages (with or without
% 3-axis accelerometers) and 2 analog channels (song & efference copy) per 
% headstage and NO digital channels.
% 
...
%
%   headstage_to_extract is a 1xN array of boolean values, where N is the number of
%       headstages in the .rhd files. Each value in the array indicates
%       whether or not the data recorded from that headstage is to be
%       extracted into binary files.Consider an example where you recorded
%       from 4 headstages but realized that only headtsages 1 and 3 have
%       neurons, while 2 and 4 don't and so it's not necessary to extract the 
%       data,then the array would be [1 0 1 0]
%  
%   audio_threshold is a 1xN array of values, where N is the number of
%       headstages in the .rhd files. Each value in the array is the
%       threshold to be used for the analog channel corresponding to
%       'audio/song' detection for that headstage. Since this value is
%       different for different boxes, we can account for that difference
%       by using this array
% 
...
%
% Version: 1.0
% Author:  Intan Technologies, Anindita Das, Brian Kardon
...
for j = 1:numel(Intan_file_dir)    % goes through all rhd files in one directory (one recording session)
    ...
    headstage_ind = find(headstage_to_extract==1);
    for count = 1:length(headstage_ind)
        n = headstage_ind(count);
        fprintf('\tHandling headstage %d of %d\n', n, total_headstage);

        path_output = sprintf('%s%sHeadstage%d', path_input, '\Data_extracted\',n);   %new directory where extracted files will be stored
        ...
        while k < numel(data.board_adc_data(2*n-1,:))   %going through the analog channel for song
            hour=hour_first;
            minute=minute_first;
            second=second_first;
            
            if abs(data.board_adc_data((2*n-1),k)) > audio_threshold(n) %|| abs((data.amplifier_data(1,k)*unit)) > 0.003  %checks for stims if it is NOT a song file
...
%                 fprintf ('rhd file number being analyzed = %f\n', file_count);
                ...
                if k > (numel(data.board_adc_data(2*n-1,:))- buffer_end*fs) %&& k >(numel(data.board_adc_data(2*n-1,:))- buffer_start*fs)
                    songData.data = data.board_adc_data((2*n-1),(k-(buffer_start*fs)):(numel(data.board_adc_data(2*n-1,:))));
                    effCopyData.data = data.board_adc_data((2*n),(k-(buffer_start*fs)):(numel(data.board_adc_data(2*n,:))));
                elseif k <(buffer_start*fs+1)
                    songData.data = data.board_adc_data((2*n-1),1:(k+(buffer_end*fs)));
                    effCopyData.data = data.board_adc_data((2*n),1:(k+(buffer_end*fs)));
                else
                    songData.data = data.board_adc_data((2*n-1),(k-(buffer_start*fs)):(k+(buffer_end*fs)));
                    effCopyData.data = data.board_adc_data((2*n),(k-(buffer_start*fs)):(k+(buffer_end*fs)));
                end
                ...
                Channel_num_all = extractfield(data.amplifier_channels,'chip_channel')';
                Headstage_name_all = extractfield(data.amplifier_channels,'port_prefix')';
                Headstage_name_prefix = unique(Headstage_name_all,'sorted');
                Channel_index = find(ismember(Headstage_name_all,Headstage_name_prefix(n)) & ismember(Channel_num_all,(8:23))); %8:23 is based off active Intan chan index for 16 channel headstages
                
              %  for chan = (channel_count*(n-1)+1):(channel_count*n)
                for chan = 1:length(Channel_index)
%                      fprintf('\t\tWriting for amplifier: %s \n', cell2mat(Headstage_name_prefix(n)));
%                      fprintf('\t\tWriting for amplifier index: %f \n', Channel_index(chan));
                    hour=hour_first;
                    minute=minute_first;
                    second=second_first+data.t_amplifier(k);    %update time
                    if second >= 60
                    ...
                    file_name = sprintf('d0000%03u_%sT%s_chan%d.nc',motif_number_array(n),date_string, time_string,chan);
                    ...
                    if k > (numel(data.board_adc_data(2*n-1,:))- buffer_end*fs) %&& k >(numel(data.board_adc_data(2*n-1,:))- buffer_start*fs)
                        otherChannel.data = data.amplifier_data(Channel_index(chan),(k-(buffer_start*fs)):(numel(data.board_adc_data(2*n-1,:))))*unit;
                    elseif k <(buffer_start*fs+1)
                        otherChannel.data = data.amplifier_data(Channel_index(chan),1:(k+(buffer_end*fs)))*unit;
                    else
                        otherChannel.data = data.amplifier_data(Channel_index(chan),(k-(buffer_start*fs)):(k+(buffer_end*fs)))*unit;
                    end
