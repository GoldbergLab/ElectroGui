function [spiketimes,ifr,indx dbase]=dbaseGetRaster(dbase, varargin);
...
for f=1:length(dbase.Times)-1
    if (filestarttimes(f)+dbase.FileLength(f)/fs) > filestarttimes(f+1) & filestarttimes(f+1)>0 & filestarttimes(f)>0
...
for i=1:size(dbase.EventTimes{indx},2)
    if ~isempty(dbase.EventTimes{indx}{1,i}) %if there are any spikes in the file
        ...
        tempspiketimes=dbase.EventTimes{indx}{1,i}(find(dbase.EventIsSelected{indx}{1,i}.*dbase.EventIsSelected{indx}{end,i}));
