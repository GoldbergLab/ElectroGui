...
for f=3:length(contents);
    clearvars -except fold i y contents f
    f-2
    file = [fold{y} contents(f).name];
    load(file);
    ...
    ptile = 10;
    ptileISI = prctile(dbase.boutISI, ptile);
    ...
    for i=1:length(eventOnsetsEscape)
        spksno(i) = sum(eventOnsetsEscape{i} > fdbkstartimemedian &...
            eventOnsetsEscape{i} < fdbkstartimemedian + 0.15);
        
    end
    [spkssort, spkssortind] = sort(spksno);
    eventOnsetsEscapeSort = eventOnsetsEscape(spkssortind);
    
    for i=1:length(eventOnsetsEscapeSort)
        spks=eventOnsetsEscapeSort{1,i};
        spksptile = spks([diff(spks)<ptileISI false]);
        
        if ~isempty(spksptile)
            for j=1:length(spksptile)
                l = line([spksptile(j)',spksptile(j)'],[yline+i-1,yline+i-1+lineheight],'color','m');
                set(l,'LineWidth', 4)
            end
        end
