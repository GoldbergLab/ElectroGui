...
xcorrLength = 20;
reducedFFTWeight = 1;
reductionFactor = 20;  % Must be a factor of fftLength
...
reducedFFTs = squeeze(mean(reshape(ffts, numSyllables, reductionFactor, fftLength / reductionFactor), 2));
...
for clusterNum = uniqueClusterIdx
    clusterTitles = titles(clusterIdx==clusterNum);
    titleCounts = [];
    for k = 1:numCategories
        title = categories{k};
        titleCounts(k) = sum(strcmp(title, clusterTitles));
    end
    [~, idx] = max(titleCounts);
...
clusterDistances = zeros([1, numSyllables]);
for c = 1:numCategories
    for s = 1:numSyllables
        clusterDistances(s) = D(s, clusterIdx(s));
%                fprintf('%s dist %0.03f / %0.03f cluster #%d: %s\n', titles{s}, clusterDistances(s), mean(clusterDistances(clusterIdx==clusterIdx(s))), clusterIdx(s), sort([titles{clusterIdx==clusterIdx(s)}], 'descend'));
    end
end
meanClusterDistances = zeros([1, numCategories]);
for c = 1:numCategories
    meanClusterDistances(c) = mean(clusterDistances(clusterIdx==c));
end
normalizedClusterDistances = zeros([1, numCategories]);
for s = 1:numSyllables
    normalizedClusterDistances(s) = clusterDistances(s) / meanClusterDistances(clusterIdx(s));
end
fprintf('Rejecting %d syllables\n', length(clusterIdx(normalizedClusterDistances>2)));
clusterIdx(normalizedClusterDistances>maxMeanClusterDistance) = nan;
