function [catsilentisis catcallisis silentspiketimes callspiketimes silenttime]=vgm_dbaseSilentCallsISI(dbase, varargin)
...
if isempty(varargin)
    ...
    [spiketimes,ifr]=vgm_dbaseGetRaster(dbase, varargin{1});
end
...
if strcmp(dbase.title(end-6:end-4),'BOS')
    BOS = 0;
end
...
for j=1:length(dbase.Times)
    ...
    % skip all BOS playback files for the purpose of singing/nonsinging stats.
    if BOS~=0 && dbase.Properties.Values{j}{BOS}
        continue
    end
    ...
    if ~isempty(spiketimes{j}) && dbase.Properties.Values{j}{unusables}
        ...
        callstart{j}=[]; callend{j}=[]; callspiketimes{j,1}=[]; silentspiketimes{j,1}=[]; 
        catcallspikes{j}=[]; catsilentspikes{j}=[];
        catcallisis{j,1}=[];
        catsilentisis{j,1}=[];
        silenttime{j,1}=[];
        calltime{j,1}=[];
