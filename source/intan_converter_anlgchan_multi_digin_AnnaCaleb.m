...
path_input = ...
    uigetdir('Z:\AIV_16ch_ephys\3179_0363\rhd\4_17_2021');  % directory where the rhd files to be analysed are stored
...
%                 fprintf ('rhd file number being analyzed = %f\n', file_count);
                ...
                if k > (numel(board_adc_data(n,:))- buffer_end*fs) %&& k >(numel(board_adc_data(n,:))- buffer_start*fs)
                    fprintf(songfileID,'%f\r\n', board_adc_data((n),(k-(buffer_start*fs)):(numel(board_adc_data(n,:)))));
                    fprintf(effcopyfileID,'%f\r\n', board_dig_in_data((n),(k-(buffer_start*fs)):(numel(board_adc_data(n,:)))));
                elseif k <(buffer_start*fs+1)
                    fprintf(songfileID,'%f\r\n', board_adc_data((n),1:(k+(buffer_end*fs))));
                    fprintf(effcopyfileID,'%f\r\n', board_dig_in_data((n),1:(k+(buffer_end*fs))));
                else
                    fprintf(songfileID,'%f\r\n', board_adc_data((n),(k-(buffer_start*fs)):(k+(buffer_end*fs))));
                    fprintf(effcopyfileID,'%f\r\n', board_dig_in_data((n),(k-(buffer_start*fs)):(k+(buffer_end*fs))));
                end
                ...
                if (acc_present == 1) && (acc_array(n)==1)
                    ...
                    elseif k <(buffer_start*fs+1)
                        fprintf(accfile1ID,'%f\r\n', aux_input_data((3*n_acc-2),1:(k_aux+(buffer_end*fs_aux))));
                        fprintf(accfile2ID,'%f\r\n', aux_input_data((3*n_acc-1),1:(k_aux+(buffer_end*fs_aux))));
                        fprintf(accfile3ID,'%f\r\n', aux_input_data((3*n_acc),1:(k_aux+(buffer_end*fs_aux))));
                    else
                ...
                for i = (channel_count*(n-1)+1):(channel_count*n)
                    ...
                    if k > (numel(board_adc_data(n,:))- buffer_end*fs) %&& k >(numel(board_adc_data(n,:))- buffer_start*fs)
                        fprintf(fileID,'%f\r\n', amplifier_data(i,(k-(buffer_start*fs)):(numel(board_adc_data(n,:))))*unit);
                    elseif k <(buffer_start*fs+1)
                        fprintf(fileID,'%f\r\n', amplifier_data(i,1:(k+(buffer_end*fs)))*unit);
                    else
