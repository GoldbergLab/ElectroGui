function [motifstarts motifends motifdurs] = dbaseGetNamedMotifs(dbase, motif);
...
for i=1:length(dbase.Times)
    if max(dbase.SegmentIsSelected{i});
        if ~isempty(regexp(concatenateMotifString(dbase.SegmentTitles{i}),motif))
            motifStartNdx=regexp(concatenateMotifString(dbase.SegmentTitles{i}),motif);
