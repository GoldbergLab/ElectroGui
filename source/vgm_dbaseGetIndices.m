function dbase=vgm_dbaseGetIndices(dbase)
...
for i = 1:length(dbase.EventDetectors)
    ...
    if strcmp(dbase.EventFunctions{i},'(Raw)');
        ...
        if length(stimindx>1);%this loop below considers cases where there are more than one Raw used
            ...
            for j=1:length(stimindx)
                if strcmp(chan,dbase.EventSources{stimindx(j)}(end))
    ...
    if strcmp(dbase.EventFunctions{i},'FIRBandPass') || strcmp(dbase.EventFunctions{i},'FIRBandPass2');
        ...
        if length(indx)>1;%this loop below considers cases where there are more than one FIR bandpasses used
            chan=dbase.title(4+strfind(dbase.title,'chan'));
            firbandx = dbase.title(3+strfind(dbase.title,'Fir'));
            for j=1:length(indx)
                if strcmp(chan,dbase.EventSources{indx(j)}(end))
                    if isempty(firbandx)
                        dbase.indx=indx(j);
    ...
    if strcmp(dbase.EventFunctions{i},'Multiunit_plot');
