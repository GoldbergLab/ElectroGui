...
dbase_dir = 'X:\Budgie\AAC_sorted_dbases_cbj\full_segments'; 
files = dir([dbase_dir '\dbase0880*.mat']); % HARD CODED BIRD ID ONLY TAKING 0880
names = {files.name};
afs = 5000;
...
for i = 1:length(names)
    ...
    if ~isfield(dbase,'accmeans') || ~isfield(dbase.accmeans,'threshold')
        ...
        all_mov = [];
        for k = 1:n:length(xfiles)
            disp(['dbase ' num2str(i) ' of ' num2str(length(names)) ' File ' num2str(k) ' of ' num2str(length(xfiles))])
            movex = cj_txtread_datonly([path '\' xfiles{k}]);
            movey = cj_txtread_datonly([path '\' yfiles{k}]);
            movez = cj_txtread_datonly([path '\' zfiles{k}]);
            if any([length(movey)~=afs*20 length(movex)~=afs*20 length(movez)~=afs*20])
                continue
            end
    ...
    %     title(names{i})
    %dbase.accmeans.allmove = []; %bad idea it makes dbases too big
