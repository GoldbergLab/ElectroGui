    ...
    methods (Static)   % dbase manipulation methods
        ...
        function dbase = InitializeDbase(settings, options)
            ...
            if options.IncludeHelp
                ...
                dbase.help.Notes = 'A 1xN cell array of character arrays, one per file. Each entry contains user-entered notes for that file.';
                ...
                dbase.help.ChannelFiles = 'A 1xC cell array of struct arrays (each of which are Nx1), one for each non-sound channel (channels 1 - C, excluding 0, which is typically the sound channel). Each struct array contains information about the files, of the same form returned by the built in ''dir'' function.';
                dbase.help.SoundLoader = 'The name of the egl_* loader plugin used to load the sound files (channel 0).';
                dbase.help.ChannelLoader = 'A 1xC cell array containing the names of the egl_* loader plugins used to load each of the non-sound channel (channels 1-C) files.';
                dbase.help.Fs = 'The sampling rate of channel 0 in Hz. Note that other channels may have different sampling rates (see ChannelFs).';
                dbase.help.ChannelFs = 'A 1xC array of sampling rates in Hz, one per non-sound channel. NaN means the sampling rate has not yet been determined (will be set when the channel is first loaded).';
                dbase.help.ChannelInfo = 'A struct array describing all available channels (real and pseudo). Each entry has fields: Name, Number, Type, IsPseudoChannel, PseudoChannelInfo.';
                dbase.help.SegmentThresholds = 'A 1xN array of threshold values used for segmenting channel 0 (sound) into segments (syllables). Inf means the threshold has not been set for that file.';
                dbase.help.SegmentTimes = 'A 1xN cell array containing Sx2 arrays of segment (syllable) start and end times in samples. For example, dbase.SegmentTimes{11}(7, 1) would give you the start sample of segment #7 in file #11.';
                dbase.help.SegmentTitles = 'A 1xN cell array of 1xS cell arrays. Each sub-cell array contains the titles (labels) for each segment. For example, dbase.SegmentTitles{11}{7} would give you the title of segment #7 in file #11.';
                dbase.help.SegmentIsSelected = 'A 1xN cell array of 1xS logical arrays indicating the selected/unselected state of each segment. For example, dbase.SegmentIsSelected{11}(7) returns true if segment #7 in file #11 is selected.';
                dbase.help.MarkerTimes = 'A 1xN cell array containing Mx2 arrays of marker start and end times in samples. For example, dbase.MarkerTimes{11}(3, 1) would give you the start sample of marker #3 in file #11.';
                dbase.help.MarkerTitles = 'A 1xN cell array of 1xM cell arrays. Each sub-cell array contains the titles (labels) for each marker. For example, dbase.MarkerTitles{11}{3} would give you the title of marker #3 in file #11.';
                dbase.help.MarkerIsSelected = 'A 1xN cell array of 1xM logical arrays indicating the selected/unselected state of each marker. For example, dbase.MarkerIsSelected{11}(3) returns true if marker #3 in file #11 is selected.';
                dbase.help.MarkerTypes = 'A 1xN cell array of 1xM categorical arrays. Each categorical array contains the type of each marker. For example, dbase.MarkerTypes{11}(3) might return a category like ''HeadBob'' indicating the type of marker #3 in file #11.';
                dbase.help.EventSources = 'A 1xS cell array of channel name strings, one per event source. For example, dbase.EventSources{2} might be ''Ch3'', meaning event source #2 detects events on channel 3.';
                dbase.help.EventChannels = 'A 1xS array of channel numbers, one per event source. For example, dbase.EventChannels(2) = 3 means event source #2 operates on channel 3.';
                dbase.help.EventChannelIsPseudo = 'A 1xS logical array indicating whether each event source''s channel is a pseudo-channel (computed) rather than a real data channel.';
                dbase.help.EventFunctions = 'A 1xS cell array of filter plugin names (egf_*) applied to the channel data before event detection, one per event source.';
                dbase.help.EventFunctionParameters = 'A 1xS cell array of structs containing parameters for each event source''s filter plugin.';
                dbase.help.EventDetectors = 'A 1xS cell array of event detector plugin names (ege_*), one per event source.';
                dbase.help.EventParameters = 'A 1xS cell array of structs containing parameters for each event source''s detector plugin.';
                dbase.help.EventParts = 'A 1xS cell array of cell arrays. Each sub-cell array lists the "parts" returned by the event detector (e.g., {''Zenith'', ''Nadir''} for ege_Spikes_AA). Event parts are different aspects of the same event.';
        ...
        function [dbase, cancel] = CreateDbase(settings, rootDir, options)
            ...
            arguments
                ...
                options.RHD = false
                options.Loaders = []   % Loader plugin struct array. If empty, loaders are discovered automatically.
            end
