function [dbase] = rc_dbasespikewidth(dbase, varargin)
...
for i=1:filestep:length(spiketimes)%i is dbase filenum; not the filenum for exper
    if ~isempty(spiketimes{i}) && ~dbase.stims(i);
        ...
        % BEGIN included for surgery rig data RC edit 123118
        if isempty(signal)
            load([exper.dir dbase.ChannelFiles{str2num(chan)}(i).name]);
            signal = rec.Data;
        end
        % END included for surgery rig data RC edit 123118
        ...
        window=win;
        % compare spike detection threshold to mean val instead of 0 rc
        % 022218
        baseline = mean(data);
        if dbase.EventThresholds(spkndx,i)>baseline
...
%eliminate outlier spikes except when there's none left. RC 041017
if bEliminateOutliers
    for j=[80 100 120];
        a=prctile(spikes(:,j),[5,95]);
        acceptNdx=find(spikes(:,j)>a(1) & spikes(:,j)<a(2));
        if ~isempty(acceptNdx)
            spikes=spikes(acceptNdx,:);
        end
    end
end

% in case there's only one spike left, use that spike.  RC 073117
if size(spikes,1)>1
