function [data, sample_rate, channelInfo] = readRHDChannel(filepath, options)
% adapted from read_Intan_RHD2000_file
%
...
magic_number = fskim(fid, 1, 'uint32', true);
if magic_number ~= hex2dec('c6912702')
    error('Unrecognized file type.');
end

% Read version number.
data_file_main_version_number = fskim(fid, 1, 'int16', true);
data_file_secondary_version_number = fskim(fid, 1, 'int16', true);
...
sample_rate = fskim(fid, 1, 'single', true);
dsp_enabled = fskim(fid, 1, 'int16');
actual_dsp_cutoff_frequency = fskim(fid, 1, 'single');
actual_lower_bandwidth = fskim(fid, 1, 'single');
actual_upper_bandwidth = fskim(fid, 1, 'single');

desired_dsp_cutoff_frequency = fskim(fid, 1, 'single');
desired_lower_bandwidth = fskim(fid, 1, 'single');
desired_upper_bandwidth = fskim(fid, 1, 'single');

% This tells us if a software 50/60 Hz notch filter was enabled during
% the data acquisition.
notch_filter_mode = fskim(fid, 1, 'int16');
...
desired_impedance_test_frequency = fskim(fid, 1, 'single');
actual_impedance_test_frequency = fskim(fid, 1, 'single');
...
if ((data_file_main_version_number == 1 && data_file_secondary_version_number >= 1) ...
    ...
    num_temp_sensor_channels = fskim(fid, 1, 'int16', true);
end
...
if ((data_file_main_version_number == 1 && data_file_secondary_version_number >= 3) ...
    || (data_file_main_version_number > 1))
    eval_board_mode = fskim(fid, 1, 'int16', true);
end
...
number_of_signal_groups = fskim(fid, 1, 'int16', true);

for signal_group = 1:number_of_signal_groups
    signal_group_name = fread_QString(fid);
    signal_group_prefix = fread_QString(fid);
    signal_group_enabled = fskim(fid, 1, 'int16', true);
    signal_group_num_channels = fskim(fid, 1, 'int16', true);
    signal_group_num_amp_channels = fskim(fid, 1, 'int16');
    ...
    if (signal_group_num_channels > 0 && signal_group_enabled > 0)
        ...
        for signal_channel = 1:signal_group_num_channels
            ...
            new_channel(1).native_order = fskim(fid, 1, 'int16', true);
            new_channel(1).custom_order = fskim(fid, 1, 'int16');
            signal_type = fskim(fid, 1, 'int16', true);
            channel_enabled = fskim(fid, 1, 'int16', true);
            new_channel(1).chip_channel = fskim(fid, 1, 'int16');
            new_channel(1).board_stream = fskim(fid, 1, 'int16');
            new_trigger_channel(1).voltage_trigger_mode = fskim(fid, 1, 'int16');
            new_trigger_channel(1).voltage_threshold = fskim(fid, 1, 'int16');
            new_trigger_channel(1).digital_trigger_channel = fskim(fid, 1, 'int16');
            new_trigger_channel(1).digital_edge_polarity = fskim(fid, 1, 'int16');
            new_channel(1).electrode_impedance_magnitude = fskim(fid, 1, 'single');
            new_channel(1).electrode_impedance_phase = fskim(fid, 1, 'single');
...
if (data_present)
    ...
    for i=1:num_data_blocks
        ...
        || (data_file_main_version_number > 1))
            t_amplifier(amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fskim(fid, num_samples_per_data_block, 'int32', true);
        else
            t_amplifier(amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fskim(fid, num_samples_per_data_block, 'uint32', true);
        end
        if (num_amplifier_channels > 0)
            amplifier_data(:, amplifier_index:(amplifier_index + num_samples_per_data_block - 1)) = fskim(fid, [num_samples_per_data_block, num_amplifier_channels], 'uint16', options.Amp, 'OutputValue', 0)';
        end
        if (num_aux_input_channels > 0)
            aux_input_data(:, aux_input_index:(aux_input_index + (num_samples_per_data_block / 4) - 1)) = fskim(fid, [(num_samples_per_data_block / 4), num_aux_input_channels], 'uint16', options.Aux, 'OutputValue', 0)';
        end
        if (num_supply_voltage_channels > 0)
            supply_voltage_data(:, supply_voltage_index) = fskim(fid, [1, num_supply_voltage_channels], 'uint16', 'OutputValue', 0)';
        end
        if (num_temp_sensor_channels > 0)
            temp_sensor_data(:, supply_voltage_index) = fskim(fid, [1, num_temp_sensor_channels], 'int16', 'OutputValue', 0)';
        end
        if (num_board_adc_channels > 0)
            board_adc_data(:, board_adc_index:(board_adc_index + num_samples_per_data_block - 1)) = fskim(fid, [num_samples_per_data_block, num_board_adc_channels], 'uint16', options.ADC, 'OutputValue', 0)';
        end
        if (num_board_dig_in_channels > 0)
            board_dig_in_raw(board_dig_in_index:(board_dig_in_index + num_samples_per_data_block - 1)) = fskim(fid, num_samples_per_data_block, 'uint16', options.DigIn, 'OutputValue', 0);
        end
        if (num_board_dig_out_channels > 0)
            board_dig_out_raw(board_dig_out_index:(board_dig_out_index + num_samples_per_data_block - 1)) = fskim(fid, num_samples_per_data_block, 'uint16', options.DigOut, 'OutputValue', 0);
        end
...
if (options.Amp && num_amplifier_channels > 0)
...
if (options.Aux && num_aux_input_channels > 0)
...
%     end
% end
if (options.ADC && num_board_adc_channels > 0)
...
if (options.DigIn && num_board_dig_in_channels > 0)
...
if (options.DigOut && num_board_dig_out_channels > 0)
...
if nargout > 2
    channelInfo.num_amplifier_channels = num_amplifier_channels;
    channelInfo.num_aux_input_channels = num_aux_input_channels;
    channelInfo.num_adc_channels = num_board_adc_channels;
    channelInfo.num_dig_in_channels = num_board_dig_in_channels;
    channelInfo.num_dig_out_channels = num_board_dig_out_channels;
end
...
a = '';
length = fskim(fid, 1, 'uint32', true);
if length == uint32(hex2dec('FFFFFFFF')) %hex2num('ffffffff')
    return;
end
