function dbase = cj_add_Han_segments_modern(dbase)
    ...
    birdID = n(1:4);
    %tt = strfind(n,'_');
    date = n(strfind(n,'T')-8:strfind(n,'T')-1);
    year = date(1:4);
    month = date(5:6);
    day = date(7:8);
    ...
    for i = 1:length(soundfiles)
        ind = strfind(soundfiles{i},'_d');
        ind2 = strfind(soundfiles{i},'_');
        d_num(i) = str2num(soundfiles{i}(ind+2:ind2(end-1)-1));
    end

    % replace B with X for pathname (this is hardcoded personal preference)
    if dbase.PathName(1) ~= 'X'
        dbase.PathName(1) = 'X';
    end
    ...
    for i = 1:length(gestures)-2 %-2 cuz don't want to do containsWarb or containsCall
        ...
        if isfolder(fullpath) && exist([fullpath '\' 'segmentations_cleaned.mat'],'file')
        ...
        for j = 1:length(vidnums)
            ...
            if ~isempty(onsets{1,j}) && ~isempty(filenum) % FOR NOW DONT UPDATE MARKERS!!!! JUST THE DBASE
...
%     savedir = 'X:\Budgie\AAC_sorted_dbases_cbj';
%     %savename = ['dbase' birdID '_' date '_segs.mat'];
%     %savename = 'dbase0871_20200406_segs_chan2_FIR.mat';
%     savename = [];
