...
% Anindita Das, July 2018, edited for multiple headstages on March 2019
...
buffer_start = 3; % time (sec) before analog signal crosses threshold first time in one adc channel data file
...
for j = 1:numel(Intan_file_dir)    % goes through all rhd files in one directory (one recording session)
    ...
    %parameters
    fs = frequency_parameters.amplifier_sample_rate; % sampling frequency kHz
    ...
    for n = 1:total_headstage
        ...
        motif_number=0;
        %k_step=1; %%%%%%____ COMMENTED BY JEET _____%%%%%%%%%%%%%
        k=buffer_start*fs+1;  %%%%______ BY JEET_______%%%%%%%%%%%%%
        
        while k <= numel(board_adc_data((2*n-1),:))- buffer_end*fs %%%%______ BY JEET_______%%%%%%%%%%%%%
        %for k = ((buffer_start*fs)+1):k_step:(numel(board_adc_data((2*n-1),:))- buffer_end*fs) %%%%%%____ COMMENTED BY JEET _____%%%%%%%%%%%%%
            hour=hour_first;
            minute=minute_first;
            second=second_first;
            time=time_first;
            
            if board_adc_data((2*n-1),k) > 0.2
        ...
        k=k+k_step %%%%______ BY JEET_______%%%%%%%%%%%%%    
        end
